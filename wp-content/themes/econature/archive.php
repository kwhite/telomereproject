...
if (!have_posts()) : 
	echo '<h2>' . __('No posts found', 'econature') . '</h2>';
else : 
				...
				<?php
					...
					'<div class="cmsms_post_cont_info entry-meta">' . 
						'<span class="cmsms_post_user_name">' . 
							__('By', 'econature') . ' ' . 
							'<a href="' . get_author_posts_url(get_the_author_meta('ID')) . '" rel="author" title="' . __('Posts by', 'econature') . ' ' . get_the_author_meta('display_name') . '">' . get_the_author_meta('display_name') . '</a>' . 
						'</span>';
					...
					'<footer class="cmsms_post_footer entry-meta">' . 
						'<div class="cmsms_post_meta_info">' . 
							...
							(comments_open() ? '<a class="cmsms_post_comments cmsms-icon-comment-6" href="' . get_comments_link() . '" title="' . __('Comment on', 'econature') . ' ' . get_the_title() . '">' . get_comments_number() . '</a>' : '') . 
						'</div>' . 
						'<a class="button cmsms_post_read_more" href="' . get_permalink(get_the_ID()) . '">' . __('Read More', 'econature') . '</a>' . 
					'</footer>';
				?>
				</div>
			</article>
