		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('cmsms_search_post'); ?>>
			...
			<div class="cmsms_search_post_cont">
				...
				<footer class="cmsms_search_post_footer entry-meta">
					...
					if (comments_open()) {
						echo '<a class="cmsms_search_post_comments cmsms-icon-comment-6" href="' . get_comments_link() . '" title="' . __('Comment on', 'econature') . ' ' . get_the_title() . '">' . get_comments_number() . '</a>';
					}
					?>
					</div>
					<?php 
					echo '<a class="button cmsms_search_post_read_more" href="' . get_permalink() . '">' . __('Read More', 'econature') . '</a>'
					?>
				</footer>
			</div>
		</article>
