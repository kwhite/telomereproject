...
function cmsms_woocommerce_add_to_cart_button() {
	...
	if (
		...
		echo '<a href="' . esc_url($product->add_to_cart_url()) . '" data-product_id="' . esc_attr($product->get_id()) . '" data-product_sku="' . esc_attr($product->get_sku()) . '" class="add_to_cart_button cmsms_add_to_cart_button product_type_simple ajax_add_to_cart cmsms-icon-basket">' . __('Add to Cart', 'econature') . '</a>';
	}
	
	echo '<a href="' . get_permalink($product->get_id()) . '" data-product_id="' . esc_attr($product->get_id()) . '" data-product_sku="' . esc_attr($product->get_sku()) . '" class="cmsms_details_button cmsms-icon-menu">' . __('Show Details', 'econature') . '</a>';
}
