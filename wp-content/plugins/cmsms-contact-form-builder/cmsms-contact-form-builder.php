...
class Cmsms_Form_Builder {
	...
	function __construct() { 
		define('CMSMS_FORM_BUILDER_VERSION', '1.3.3');
		
		define('CMSMS_FORM_BUILDER_FILE', __FILE__);
		
		define('CMSMS_FORM_BUILDER_PATH', plugin_dir_path(CMSMS_FORM_BUILDER_FILE));
		
		define('CMSMS_FORM_BUILDER_URL', plugin_dir_url(CMSMS_FORM_BUILDER_FILE));
		...
		register_activation_hook(CMSMS_FORM_BUILDER_FILE, array($this, 'cmsms_form_builder_activate_deactivate'));
		
		register_deactivation_hook(CMSMS_FORM_BUILDER_FILE, array($this, 'cmsms_form_builder_activate_deactivate'));
		
		
		add_action('admin_menu', array($this, 'cmsmasters_enable_form_builder'));
		
		// Load Plugin Local File
		load_plugin_textdomain('cmsms_form_builder', false, dirname(plugin_basename(CMSMS_FORM_BUILDER_FILE)) . '/languages/');
	}
	...
	function form_builder_scripts() {
		if (!is_admin()) {
			...
			wp_register_script('cmsmsValidation', CMSMS_FORM_BUILDER_URL . 'js/jquery.validationEngine.min.js', array('jquery'), '2.6.2', true);
			
			wp_register_script('cmsmsValidationLang', CMSMS_FORM_BUILDER_URL . 'js/jquery.validationEngine-lang.js', array('jquery', 'cmsmsValidation'), CMSMS_FORM_BUILDER_VERSION, true);
			...
			wp_register_script('reCAPTCHA2', 'https://www.google.com/recaptcha/api.js?onload=validateCaptcha', array('jquery', 'cmsmsValidation'), '2.0.0', true);
		}
	}
	...
	function cmsms_form_builder_activate_deactivate() {
		flush_rewrite_rules();
	}
}
