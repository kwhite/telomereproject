	...
	if (get_post($formname) != NULL) {
		...
		foreach ($results as $form_result) {
			...
			if ($form_result->type == 'form') {
				...
				$out .= "jQuery('#form_" . $formname . " a#cmsms_" . $formname . "_formsend').click(function () { \n\t\t" . 
			...
			if ($form_result->type == 'form') {
				...
				if (in_array('captcha', $form_result->parameters)) {
					$out .= "function phpValidateCaptcha(captchaServerKey) { " . 
								'var cmsms_' . $formname . '_captcha_html = jQuery.ajax( { ' . "\n\t\t\t" . 
									"type : 'post', \n\t\t\t" . 
		...
		foreach ($results as $form_result) {
			...
			if ($form_result->type == 'form') {
				if (in_array('captcha', $form_result->parameters)) {
					...
					'<div id="form_' . $formname . '_builder_captcha" class="form_info cmsms-form-builder-captcha g-recaptcha" data-sitekey="' . $recaptcha_public_key . '"></div>' . "\n";
				}
